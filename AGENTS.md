...
`deno.json`. This is a **library**, not an app: no build step, consumers
...
- `src/tools/` — zero-dependency agent tool suite (Now, Calculator, ListDir,
  ...
  StateStore + html/net helpers). Single-file modules returning serializable
  structured results that map onto `ToolDefinition`/`ToolHandler` pairs.
- `src/embeddings/` — separate semantic-memory module (client, cache, router,
  SQLite store). Import from its `mod.ts`.
...
- `docs/` — design specs (written in Portuguese), not user-facing docs. The
  real API reference is `README.md`.
...
deno task test    # deno test --allow-ffi --allow-net --allow-read --allow-write
```
...
`examples/pipeline-tools.ts` needs broad flags (`--allow-net --allow-env
--allow-read --allow-write --allow-run`); `examples/mcp-remote.ts` needs plain
`--allow-net` because it reaches `https://mcp.exa.ai`.
...
- `deno task check` fails with 3 pre-existing lint errors and ~12 unformatted
  files. If your diff didn't touch them, don't chase them all — just don't
  make them worse.
...
- Ollama must be running locally. **No mock/offline mode exists.** For a
  remote host, set `OLLAMA_HOST` or pass a custom client (`new
  ollama.Ollama({ host })`) — see README "Running with a Different Host".
